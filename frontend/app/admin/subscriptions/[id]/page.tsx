        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="text-2xl sm:text-3xl font-bold">
              {t(PLAN_LABEL_KEYS[sub.plan_tier])}
            </h1>
          ...
          <div className="ops-card p-6">
            ...
            {sub.user_phone && <p className="text-sm mt-1">{sub.user_phone}</p>}
          </div>

          <div className="ops-card p-6">
            <h2 className="font-semibold mb-4">{t('adminSubs.breadcrumbSubs')}</h2>
            <dl className="grid sm:grid-cols-2 gap-4 text-sm">
              <Row label={t('adminSubs.plan')} value={t(PLAN_LABEL_KEYS[sub.plan_tier])} />
          ...
          <div className="ops-card p-6">
            ...
            <dl className="grid sm:grid-cols-2 gap-4 text-sm">
              <Row
                ...
                value={t(`adminSubs.${sub.payment_status === 'paid' ? 'paid' : sub.payment_status === 'failed' ? 'failed' : sub.payment_status === 'pending' ? 'pending' : 'none'}`)}
              />
              <Row
                label={t('adminSubs.method')}
                value={sub.payment_method ? t(METHOD_LABEL_KEYS[sub.payment_method] || 'adminSubs.methodOther') : '—'}
              />
              <Row label={t('adminSubs.transactionId')} value={sub.transaction_id} />
            </dl>
          </div>
          ...
          <div id="payments" className="ops-card p-6">
            ...
            {sub.payment_proofs.length === 0 ? (
              <p style={{ color: 'var(--ops-muted)' }}>{t('adminSubs.emptyProofs')}</p>
            ) : (
              <div className="space-y-3">
                {sub.payment_proofs.map((proof) => (
                  <div key={proof.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border p-3" style={{ borderColor: 'var(--ops-border)' }}>
                    <div>
                      <div className="font-medium">{formatMoney(proof.amount, proof.currency, locale)}</div>
                      <div className="text-xs" style={{ color: 'var(--ops-muted)' }}>{proof.transaction_id} · {formatOpsDate(proof.created_at, locale)}</div>
                    </div>
                    <div className="flex items-center gap-3">
                      <StatusBadge status={proof.status} t={t} />
                      {(proof.file_url || proof.file) && (
                        <a href={proof.file_url || getFullUrl(proof.file)} target="_blank" rel="noopener noreferrer" className="text-sm font-medium" style={{ color: 'var(--ops-primary)' }}>
          ...
          <div className="ops-card p-6">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <select className="ops-select" value={plan} onChange={(e) => setPlan(e.target.value as PlanTier)} disabled={acting}>
                {(['free', 'premium', 'business', 'family'] as PlanTier[]).map((p) => (
                  <option key={p} value={p}>{t(PLAN_LABEL_KEYS[p])}</option>
                ))}
              </select>
              <button type="button" className="ops-btn ops-btn-primary" disabled={acting || plan === sub.plan_tier} onClick={() => run(() => adminApi.subscriptions.changePlan(sub.id, plan))}>
                {t('adminSubs.changePlan')}
              </button>
              <button type="button" className="ops-btn ops-btn-ghost" disabled={acting} onClick={() => run(() => adminApi.subscriptions.extend30Days(sub.id))}>
                {t('adminSubs.extendDays')}
              </button>
              {sub.status === 'paused' ? (
                <button type="button" className="ops-btn ops-btn-ghost" disabled={acting} onClick={() => run(() => adminApi.subscriptions.resume(sub.id))}>
                  {t('adminSubs.resume')}
                </button>
              ) : (
                <button type="button" className="ops-btn ops-btn-ghost" disabled={acting} onClick={() => setConfirm('pause')}>
                  {t('adminSubs.pause')}
                </button>
              )}
              <button type="button" className="ops-btn ops-btn-ghost" disabled={acting} onClick={() => run(() => adminApi.subscriptions.sendReminder(sub.id, { channels: ['email', 'push', 'sms', 'whatsapp'], days: 3 }))}>
                {t('adminSubs.sendNotification')}
              </button>
              <button type="button" className="ops-btn ops-btn-ghost" disabled={acting} onClick={() => setConfirm('refund')}>
                {t('adminSubs.refund')}
              </button>
              {sub.status !== 'cancelled' && (
                <button type="button" className="ops-btn ops-btn-danger" disabled={acting} onClick={() => setConfirm('cancel')}>
                  {t('adminSubs.cancel')}
                </button>
              )}
            </div>
            ...
            {sub.audit_logs.length === 0 ? (
              ...
              <div className="space-y-2 text-sm">
                {sub.audit_logs.map((log) => (
                  <div key={log.id} className="flex flex-wrap justify-between gap-2 border-b py-2" style={{ borderColor: 'var(--ops-border)' }}>
                    <div>
                      <span className="font-medium">{log.action}</span>
                      <span className="ml-2" style={{ color: 'var(--ops-muted)' }}>{log.admin_name || log.admin_email}</span>
                    </div>
